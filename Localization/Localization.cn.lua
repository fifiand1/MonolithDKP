if GetLocale() == "zhCN" then
  ...
  core.ZoneList = {
    "熔火之心", "黑翼巢穴", "安其拉神庙", "纳克萨玛斯", "祖尔古鲁布", "安其拉废墟", "奥妮克希亚的巢穴", "世界首领"
  }
  ...
  core.L = {
    ...
    OnTimeBonus           = "准时奖金",
    BossKillBonus         = "首领击杀分",
    RaidCompletionBonus   = "全勤分",
    NewBossKillBonus      = "首领开荒分",
    ...
    SelPlayersTTDesc      = "将上述DKP衰减应用于DKP表上的 |cffff0000只要|r 选定的玩家",
    ...
    WeeklyDecay           = "每周衰变",
    AdjustDKPTTDesc       = "将以上条目应用于DKP表中的所有选定玩家。",
    AdjustDKPTTWarn       = "此项将广播给您公会中的所有在线玩家。",
    PointsTTDesc          = "在DKP表上输入要分配给选定玩家的DKP数量。可以在下面的“选项”标签中更改默认值。",
    PointsTTWarn          = "使用负数从所选玩家中删除DKP。",
    Reason                = "原因",
    ReasonTTDesc          = "选择DKP调整的原因。如果选择“ 首领杀死分 ”或“ 首领开荒分 ”，则会创建一个附加的下拉列表来选择区域和Boss。 “ 其他 ”将创建一个文本框供您输入自定义原因",
    ReasonTTWarn          = "当首领被杀死时，将自动为您选择合适的区域和首领",
    WARNING               = "警告",
    ...
    RaidDKPAdjustBy       = "活动DKP调整",
    DKPAdjustBy           = "DKP调整",
    ForReason             = "由于原因",
    ForPlayers            = "对于以下玩家",
    RaidTimerBonusConfirm = "您确定要对这次Raid应用“按时”奖金吗",
    RaidResume            = "Raid计时器已继续",
    RaidStart             = "Raid计时器已启动",
    RaidPause             = "Raid计时器已暂停",
    PauseRaid             = "暂停Raid",
    PauseRaidTTDesc       = "如果团长决定暂停计时器，则这会暂停Raid计时器",
    PauseRaidTTWarn       = "可以通过单击“ 继续Raid ”恢复",
    ...
    IncludeStandbyTTWarn  = "通过选择DKP表上不在团队中的玩家来创建备用列表，右键单击>管理备用列表>将选定的玩家添加到备用列表。",
    EndCurRaidConfirm     = "您确定要结束当前的袭击吗？",
    RaidTimerConclude     = "Raid计时器已结束",
    InitRaid              = "初始化Raid",
    InitRaidTTDesc        = "启动raid计时器，根据您在上面给出的时间增量来奖励DKP。可以由Raid中的任何人员进行初始化，但是只有Raid首领会在满足检查条件的情况下向DKP奖励。",
    InitRaidTTWarn        = "广播给所有Raid人员。每个军官都有一个计时器，但只有团队领导者才可以授予奖励。如果发生事件（脱机，重载等），导致队长失去计时器，则将Raid队长交给另一个仍在运行其计时器的军官，以防干扰。",
    ...
    LifetimeEarned        = "终身获得",
    LifetimeSpent         = "终身花费",
    NoPlayerInStandby     = "替补组中没有玩家。",
    NoPartyOrRaid         = "您没有小队或团队。",
    NoCoreRaidTeam        = "您的核心团队中没有玩家。",
    MultipleSelect        = "多项选择",
    InviteSelected        = "邀请参加Raid",
    SelectAll             = "全选",
    Views                 = "观看次数",
    TableViews            = "表格检视",
    ViewRaid              = "查看团队",
    ViewStandby           = "查看替补列表",
    ViewRaidStandby       = "查看Raid和替补",
    ViewCoreRaid          = "查看核心攻略",
    ViewAll               = "查看全部",
    ClassFilter           = "类过滤器",
    ManageLists           = "管理清单",
    ManageStandby         = "管理备用清单",
    AddToStandby          = "将选定的玩家添加到替补列表",
    RemoveFromStandby     = "从替补列表中删除选定的玩家",
    ClearStandby          = "清除备用清单",
    ManageCoreList        = "管理核心攻略列表",
    Invite                = "邀请",
    ToRaid                = "Raid",
    Remove                = "去掉",
    FromStandbyList       = "从替补列表",
    Add                   = "加",
    ToStandbyList         = "到替补列表",
    Close                 = "关",
    DKPStatus             = "DKP状态",
    ...
    DeleteEntryRefundConf = "受此条目影响的任何DKP都将从列出的每个玩家中退款/删除。",
    ...
    StaticDescription     = "使用静态项目值时，所有插槽（或单个项目）都分配有静态值。每个需要该项目的玩家（带有！bid）都将提交一个出价，并且具有最高可用DKP的玩家获胜。成本值选项为\"整数\"或\"百分 \"。\n\nEx：|cffa335ee|Hitem:19143::::::::120:577::::::|h[Flameguard Gauntlets]|h|r下降，静态值为50 DKP。每个想要的玩家都通过一个打开的通道（在下面选择）使用\"！bid \"。具有最高可用DKP的玩家获胜，并收取该静态值。阅读工具提示以获取更多信息。",
    ...
    ZeroSumDescription    = "使用零和DKP，项目可以具有静态值，也可以被投标。当玩家赢得某物品的竞标价格或成本时，该DKP将从其中扣除，并在团队中平均分配。建议通过“ DKP取整”选项使用小数位，因为取整的整数可能会导致通货膨胀或DKP丢失。这样会比成本高出9点。\n\nEx：|cffa335ee|Hitem:18824::::::::120:577::::::|h[Magma Tempered Boots]|h|r掉落并的静态费用为30 DKP。玩家A通过拥有最多的DKP（或由平局领导酌情分配（如果平局）分配）来赢得竞标，并且其DKP减少30。然后每个团队成员获得0.75 DKP。",
    ...
    Added                 = "添加",
    Players               = "玩家",
    NotAnOfficer          = "不是官员。您只能在行会设置中添加具有“编辑官员备注”权限的等级。",
    ...
    RemSelEntriesTTDesc   = "从DKP表中删除选定的玩家。该玩家的所有引用将保留（DKP历史记录和战利品历史记录），但其名称将显示为灰色。",
    RemSelEntriesTTWarn   = "警告：此操作是永久性的。",
    ConfirmRemoveSelect   = "您确定要删除吗",
    NoEntriesSelected     = "未选择任何条目。",
    ResetPrevDKP          = "重设上一个DKP",
    ResetPrevDKPTTDesc    = "将先前的DKP计数器重置为其当前DKP（每个玩家当前DKP旁边的灰色小+/-数字）。这不会以任何方式影响功能。只是自上次重置以来每个玩家的收入/花费的参考。",
    ...
    ConfirmWhitelist      = "您确定要将选定的玩家添加到白名单中吗？",
    ...
    BossKillBonusTTDesc   = "杀死首领可获得的奖金。",
    RaidCompleteBonusTT   = "奖励给参加完工团队的每个人。",
    NewBossKillTTDesc     = "Raid期间首次首领击杀的奖励。",
    UnexcusedTTDesc       = "对Raid无故缺席的处罚。",
    ...
    ForNeckSlot           = "用于颈部插槽项目。",
    ForShoulderSlot       = "用于肩部插槽项目。",
    ForBackSlot           = "用于背部插槽项目。",
    ForChestSlot          = "用于胸部插槽项目。",
    ForWristSlot          = "用于护腕插槽项目。",
    ForHandSlot           = "用于手插槽项目。",
    ForWaistSlot          = "用于腰带插槽项目。",
    ForLegSlot            = "用于腿部插槽项目。",
    ForFeetSlot           = "用于脚部插槽项目。",
    ForFingerSlot         = "用于手指插槽项目。",
    ForTrinketSlot        = "用于饰品插槽项目。",
    ForOneHandSlot        = "单手武器。",
    ForTwoHandSlot        = "双手武器。",
    ForOffHandSlot        = "用于副手物品（盾牌，施法者副手）。",
    ForRangeSlot          = "用于远程插槽物品（弓，枪，魔杖，遗物）。",
    ...
    DistrubuteBanked      = "在当前首领的抢劫完成后，将银行化DKP平均分配给所有团队成员。",
    IncludeStandbyList    = "包括备用清单",
    IncStandbyListTTDesc  = "在分发中的替补列表中包括玩家。",
    ...
    AddTarget             = "添加目标",
    NotInRaidFilter       = "不在团队",
    Online                = "线上",
    OnlyPartyRaid         = "只有小队或团队",
    ManuallyDenied        = "已被拒絕。",
    UsePercentage         = "使用百分比",
    RollRange             = "roll范围",
    AwardItem             = "奖励项目",
    ...
    BestPractices         = "最佳实践：最好在每个人划入区域后，在袭击开始时（或如果有新玩家加入），使用“管理”选项卡底部的按钮让一名军官（具有最新的表）广播所有数据。 （分区可能会中断广播）。只有一名警官需要这样做。之后，创建的每个条目都会单独广播。除了使每个人都在同一列表中之外，出于任何原因都不需要全表广播。完整广播不会合并表。它会用您的确切副本完全覆盖行会表中的每个人。还建议您在每个dkp / loot会话之后/重新加载UI，以将数据提交到SavedVariables文件。如果您不重新加载就进行了一次完整的Raid，最后崩溃或断开连接，则所有已应用的数据都将丢失。在每个首领之后重新加载可以帮助防止这种情况。此外，在其他人员离线时编辑表（授予DKP或删除条目以测试插件），尽管具有完全相同的数据，也会使它们的表过时。如果发生这种情况，请从“公会领袖”公共笔记中删除{MonDKP=#####}标记以重置时间戳。",
